    ...
    <div className="options-content">
      ...
      <section id="provider"><div className="section-title"><span>01</span><div><h2>Provider & API key</h2><p>Rewrite requests go directly from the extension to your selected provider.</p></div></div>
        <div className="settings-card">
          ...
          {validation && !['idle', 'checking', 'valid'].includes(validation) && <div className="ui-status ui-status--error">{validation === 'invalid_key' ? 'Gemini rejected this key. Check that it was copied completely and has API access.' : validation === 'quota' ? 'This key is valid, but its quota is currently exhausted.' : validation === 'network' ? 'Chrome could not reach Gemini. Check browser, VPN, or firewall access.' : validation === 'request' ? 'Gemini rejected the verification request. Reload Ondrift and try again.' : validation === 'unavailable' ? 'Gemini is temporarily unavailable for this project, including the fallback model.' : 'The key could not be verified.'}</div>}
        </div>
      </section>
