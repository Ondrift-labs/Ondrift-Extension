...
describe("handleRuntimeRequest", () => {
  ...
  it("rejects disabled sites before calling a provider", async () => {
    const deps = dependencies({
      settings: { get: vi.fn(async () => ({ provider: "gemini", apiKeys: { gemini: "key" }, enabledSites: { chatgpt: false, claude: true }, onboardingComplete: true, persona: "general", saveHistory: true, consentGranted: true })) } as never,
    });
  ...
  it("does not persist history before consent", async () => {
    const deps = dependencies({
      settings: { get: vi.fn(async () => ({ provider: "gemini", apiKeys: {}, enabledSites: { chatgpt: true, claude: true }, onboardingComplete: false, persona: "general", saveHistory: true, consentGranted: false })) } as never,
    });
  ...
  it("does not treat completed onboarding as history consent", async () => {
    const deps = dependencies({
      settings: { get: vi.fn(async () => ({ provider: "gemini", apiKeys: {}, enabledSites: { chatgpt: true, claude: true }, onboardingComplete: true, persona: "general", saveHistory: true, consentGranted: false })) } as never,
    });
